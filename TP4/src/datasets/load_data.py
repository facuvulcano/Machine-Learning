...
def load_and_prepare_data(train_data_path='', val_data_path=''):
    ...
    if train_data_path and val_data_path:
        ...
        X_train = train_df.drop('Precio', axis=1)
        y_train = train_df['Precio']
        X_val = val_df.drop('Precio', axis=1)
        y_val = val_df['Precio']

        min_max_values = {
            feature : (X_train[feature].min(), X_train[feature].max()) for feature in features_to_normalize
        }
